        ...
        if action == 'update':
            ...
            if form.cleaned_data['payment_id'] == '':
                ...
                if int(form.cleaned_data['schedule_frequency']) == PaymentScheduleFrequency.objects.get(name__exact='Linked to Other Payment').id:
                    ...
                    p.parent_payment=mp
                    p.offset=form.cleaned_data['offset']
...
def update_payment_classification(request):
    try:
        ...
        # response_data['form_data'] = form     # Don't think this is required
...
def generate_calendar_view(request):
    ...
    try:
        ...
        for p in Payment.objects.filter(owner=request.user, active=True):
            ...
            logger.debug('generate_calendar_view: processing payment "%s", occurrences: %d'
                         % (p.title, p.schedule.occurrences))
            ...
            if rr:
                for i in range(0, len(rr)):
                    ...
                    if payment_date in exclusions:
                        continue
                    if p.parent_payment is not None:
                        kwargs = {}
        ...
        for pc in sorted_payment_calendar:
            ...
            if pc['in_out'] == 'i':
                curr_budget_balance += pc['amount_value']
...
def bank_account_view(request):
    ...
    if request.method != 'POST':
        bank_accounts = BankAccount.objects.filter(owner=request.user, active=True)
    ...
    try:
        ...
        logger.debug('bank_account_view:- request.POST: %s' % jsonpickle.encode(request.POST))
        ...
        bank_accounts = BankAccount.objects.filter(owner=request.user, active=True).all()
        return render_to_response('budget/snippet__bank_accounts.html', {'bank_accounts': bank_accounts,
...
def get_accounts_json(request):
    ...
    try:
        ...
        accounts = BankAccount.objects.filter(owner=request.user, active=True).order_by('pk').all()
        accounts_json = []
        for a in accounts:
...
def generate_calendar_search_terms(owner):
    ...
    for p in Payment.objects.filter(owner=owner, active=True).all():
        ...
        if p.category.name not in calendar_search_terms:
...
def schedule_rrule(payment):
    ...
    schedule = payment.schedule if payment.parent_payment is None else payment.parent_payment.schedule
    logger.debug('schedule_rrule:- schedule.frequency.name: %s' % schedule.frequency.name)
    if schedule.frequency.name == 'Monthly':
        kwargs['freq'] = MONTHLY
