...
function initCalendarView(user_language) {
    // hook up events
    ...
    calendar_table = $('#table__calendar').DataTable({
        ...
        "columns": [
            ...
            },
            {
                ...
                className: "calendar__payment_category dt-nowrap calendar__searchable"
            },
            {
                data: 'title',
...
var calendarLoadComplete = function( settings, json ) {
    ...
    bank_balance__min = JSON.parse(json.json["bank_balance__min"]);
...
//    new AutoNumeric.multiple('#table__calendar tbody .money', AutoNumeric.getPredefinedOptions().NorthAmerican);
    $('#table__calendar tbody .money').autoNumeric('init', {currencySymbol: '$'} );
    ...
    $('td.calendar__payment_category').click(_.debounce(editPaymentCategory, MILLS_TO_IGNORE_CLICKS, true));
...
//    if (is_money) {
////        $this_money = new AutoNumeric($this[0], [AutoNumeric.getPredefinedOptions().NorthAmerican, autoNumericOptions]);
////        this_val = $this_money.getNumericString();
//        this_val = $this.text();
//    } else {
//        this_val = $this.text();
//    }

    var $input_element = $('<input />',{'value' : this_val});
    if (is_money) $input_element.autoNumeric('init', {currencySymbol: '$'} );
        ...
        .append( $('<button />', {'text' : 'Update This', 'class' : 'button-update-detail-save-single', 'data-payment_id' : payment_id})
            ...
            .click(_.once(function (event) {
                ...
                var payload = "payment_id=" + payment_id + "&field=" + field_name + "&single_series=single&value="
                    + ((is_money) ? $(this).siblings('input').getNumericString() : $(this).siblings('input').val())
...
//        if(e.keyCode == 13) $(this).trigger("enterKey");
            if(e.keyCode == 27) { // esc
                $(this).siblings('.button-update-detail-cancel').click();
            };
        }));
...
var editCalendarAccount = function(event) {
    ...
    $.getJSON(URL__GET_ACCOUNTS_JSON, function(result) {
        ...
        var og_debit = $('<optgroup label="DEBIT ACCOUNTS" />');
        var og_credit = $('<optgroup label="CREDIT ACCOUNTS" />');
        var og_virtual = $('<optgroup label="VIRTUAL ACCOUNTS" />');
        for (var i = 0; i < account_list.length; i++) {
            var o;
            if (account_list[i]['title'] === calendar_editing_current_val) {
                o = $('<option />', {value: account_list[i]['id'], text: account_list[i]['title'], selected: 'selected'});
            } else {
                o = $('<option />', {value: account_list[i]['id'], text: account_list[i]['title']});
            }
            o.appendTo( (account_list[i]['type'] === 'debit') ? og_debit
                : ((account_list[i]['type'] === 'credit') ? og_credit
                : og_virtual));
        }
        og_debit.appendTo(s);
        og_credit.appendTo(s);
        og_virtual.appendTo(s);
...
//                    console.info("calendarUpdatePaymentDetails:- serverResponse_data: " + serverResponse_data);
                    ...
                    $(calendar_editing_target_element).find('.button__calendar_edit__classification_cancel')
                        ...
                        .click(_.debounce(function (event) {
                            $(this).siblings('button').button( "disable" );
                            $(this).closest('tr')
                                .on('click', 'td.calendar__payment_category',
                                    _.debounce(editPaymentCategory, MILLS_TO_IGNORE_CLICKS, true));
...
//        })
//        .done(function(serverResponse_data) {
//            console.debug("calendarUpdatePaymentInline:- back from view: " + serverResponse_data);
//            try {
//                if (serverResponse_data['result_success'] === 'pass') reloadCalendar();
//                user_message(serverResponse_data['result_success'], serverResponse_data['result_message']);
//                refresh_calendar_search_terms(serverResponse_data['search_terms']);
...
var processCalendarMarkAsPaidReceived = function () {
    ...
    .done(function (server_response) {
        if (server_response['result_success'] == 'pass') {
            ...
            calendar_end_date = new Date(server_response['calendar_end_date']);
            calendar_final_balance = server_response['final_balance'];
            bank_balance__min = server_response['bank_balance__min'];
        } else {
