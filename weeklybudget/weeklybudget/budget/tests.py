...
class TestGenerateCalendarView(TestCase):
    ...
    def test_calendar_view_setup(self):
        ...
        bank_balance__min = jsonpickle.decode(response_data['bank_balance__min'])
        ...
        pc1 = (p_item for p_item in response_data['data'] if p_item["account"] == "john account"
               and p_item["payment_date"] == "2016-06-21" ).next()
        self.assertEqual(float(pc1['bank_balance']['john account']), 2885, pc1)
        self.assertEqual(float(bank_balance__min['john account']['amount']), 885)
        self.assertEqual(bank_balance__min['john account']['payment_date'], '2016-06-02',
                         response_data['bank_balance__min'])

        # p_list = [item for item in response_data['data'] if item['account'] == 'john credit account']
        # self.assertEqual(float(p_list[0]['bank_balance']['john credit account']), 1885, p_list)

        pc1 = (p_item for p_item in response_data['data'] if p_item["account"] == "john credit account"
               and p_item["payment_date"] == "2016-11-01").next()
        self.assertEqual(float(pc1['bank_balance']['john credit account']), 800, pc1)
        self.assertEqual(float(bank_balance__min['john credit account']['amount']), 400)
        self.assertEqual(bank_balance__min['john credit account']['payment_date'], '2017-09-01',
                         response_data['bank_balance__min'])
...
class TestBankAccount(TestCase):
    ...
    def test_create_bank_account(self):
        ...
        bas = list(ba for ba in response.context["bank_accounts"] if ba.title == 'Debit Account #1')
        self.assertEqual(1, len(bas))
        self.assertEqual(bas[0].display_order, 2)

        response = c.post('/budget/bank_account/', {'action': 'new', 'account_type': 'debit'})
        self.assertEqual(len(response.context["bank_accounts"]), 5, response.context["bank_accounts"])
        self.assertEqual(response.context["result_success"], "pass", response.content)
        bas = list(ba for ba in response.context["bank_accounts"] if ba.title == 'Debit Account #2')
        self.assertEqual(1, len(bas))
        self.assertEqual(bas[0].display_order, 3)

        response = c.post('/budget/bank_account/', {'action': 'new', 'account_type': 'credit'})
        self.assertEqual(len(response.context["bank_accounts"]), 6)
        self.assertEqual(response.context["result_success"], "pass", response.content)
        bas = list(ba for ba in response.context["bank_accounts"] if ba.title == 'Credit Account #1')
        self.assertEqual(1, len(bas))
        self.assertEqual(bas[0].display_order, 2)

        response = c.post('/budget/bank_account/', {'action': 'new', 'account_type': 'virtual'})
        self.assertEqual(len(response.context["bank_accounts"]), 7)
        self.assertEqual(response.context["result_success"], "pass", response.content)
        bas = list(ba for ba in response.context["bank_accounts"] if ba.title == 'Virtual Account #1')
        self.assertEqual(1, len(bas))
        self.assertEqual(bas[0].display_order, 2)
    ...
    def test_update_bank_account(self):
        ...
        res = (item for item in response.context["bank_accounts"] if item.pk == ba.pk).next()
        ...
        response = c.post('/budget/bank_account/', payload)
        res = (item for item in response.context["bank_accounts"] if item.pk == ba.pk).next()
        self.assertEqual(res.account_type, 'virtual')
        ...
        response = c.post('/budget/bank_account/', payload)
        res = (item for item in response.context["bank_accounts"] if item.title == 'john account - updated!').next()
        self.assertEqual(res.display_order, 3)
        res = (item for item in response.context["bank_accounts"] if item.title == 'john account 2').next()
        self.assertEqual(res.display_order, 1)
        res = (item for item in response.context["bank_accounts"] if item.title == 'john account 3').next()
        self.assertEqual(res.display_order, 2)
    ...
    def test_delete_bank_account(self):
        ...
        response = c.post('/budget/bank_account/', payload)
        self.assertIsNone(response.context, 'response.context is None. response.content: %s' % response.content)
        self.assertEqual(jsonpickle.decode(response.content)["result_success"], "fail")

        for p in Payment.objects.filter(account=ba):
            p.active = False
            p.save()
        response = c.post('/budget/bank_account/', payload)
        self.assertIsNotNone(response.context, 'response.context is None. response.content: %s' % response.content)
...
class TestUpdatePaymentDate(TestCase):
    ...
    # TODO test one-off payment exclusion with occurrence-based payment to ensure end date remains the same
    ...
    def test_create_custom_payment_date_new_payment_in_calendar(self):
        ...
        pc = (item for item in response_data["data"] if item["title"] == "Payment 1 - 23/06/2016").next()
        self.assertEqual(pc['payment_date'], '2016-06-23')
        pc = [item for item in response_data["data"] if item["title"] == "Payment 1"]
        self.assertEqual(pc[0]['payment_date'], '2016-06-07')
        self.assertEqual(pc[1]['payment_date'], '2016-07-05')
    ...
    def test_update_to_linked_payment(self):
        ...
        response_data = jsonpickle.decode(response.content)
        self.assertNotIn("Exception", response_data, 'Exception occurred: %s' % response_data)
        self.assertEqual(response_data["result_success"], "fail")

        payload["linked_to"] = Payment.objects.get(title__exact='Annual Payment').id
        response = c.post('/budget/update_payment_date/', payload)
        # print response.content
        response_data = jsonpickle.decode(response.content)
        self.assertEqual(response_data["result_success"], "pass")
        ...
        self.assertTrue(Payment.objects.exclude(parent_payment__isnull=True).exists())
        self.assertIsNotNone(p.parent_payment)
        self.assertEqual('Annual Payment', p.parent_payment.title)
        self.assertEqual(-1, p.offset)
        self.assertEqual('days', p.offset_type)
        self.assertEqual(PaymentScheduleFrequency.objects.get(name__exact='Linked to Other Payment').id,
                         p.schedule.frequency.id)
        self.assertEqual('2016-06-02', str(p.parent_payment.schedule.next_date))
        self.assertEqual('2021-06-01', str(p.schedule.next_date))
        self.assertIsNone(p.schedule.end_date)
        self.assertEqual(0, p.schedule.occurrences)
...
class MarkPaidReceivedTest(TestCase):
    ...
    def test_mark_paid_of_exclusion_payment(self):
        c = Client()
        self.assertTrue(c.login(username='john', password='johnpassword'))
        p = Payment.objects.get(title__exact='Payment 1')
        payload = {
        ...
        pe = PaymentScheduleExclusion.objects.get(main_payment=p).exclusion_payment
        payload = {
            'action': 'update',
            'payment_id': pe.pk,
            'payment_date': '2016-06-23',
            'start_date': '2000-01-01',
            'end_date': '2017-01-01',
        }
        response = c.post('/budget/calendar_view/', payload)
        response_data = jsonpickle.decode(response.content)
        self.assertNotIn("Exception", response_data, 'Exception occurred: %s' % response_data)
        self.assertFalse(Payment.objects.filter(active=True, title__exact=pe.title).exists())

    def test_mark_paid_of_end_dated_payment(self):
        ps = PaymentSchedule.objects.create(
            next_date=datetime(2016, 9, 7),
            frequency=PaymentScheduleFrequency.objects.get(name__exact='Weekly'),
            weekly_dow_wed=True,
            weekly_frequency=1,
            end_date=datetime(2016, 9, 21),
        )
        ps.save()
        p = Payment.objects.create(
            title='Test Payment',
        ...
        # Mark middle payment as complete
        c = Client()
        self.assertTrue(c.login(username='john', password='johnpassword'))
        payload = {
            'action': 'update',
            'payment_id': p.pk,
            'payment_date': '2016-09-14',
        ...
        response_data = jsonpickle.decode(response.content)
        self.assertNotIn("Exception", response_data, 'Exception occurred: %s' % response_data)
        pc = [item for item in response_data['data'] if p.title in item['title']]
        self.assertEqual(2, len(pc))

        # Mark last payment as complete
        payload = {
            'action': 'update',
            'payment_id': p.pk,
            'payment_date': '2016-09-21',
            'start_date': '2016-01-01',
            'end_date': '2017-01-01',
        }
        response = c.post('/budget/calendar_view/', payload)
        ...
        # Mark remaining payment as complete
        payload = {
            'action': 'update',
            'payment_id': p.pk,
        ...
        self.assertNotIn("Exception", response_data, 'Exception occurred: %s' % response_data)
        pc = [item for item in response_data['data'] if p.title in item['title']]
        self.assertEqual(0, len(pc))

    def test_mark_paid_of_payment_with_occurences(self):
        ps = PaymentSchedule.objects.create(
            next_date=datetime(2016, 9, 7),
            frequency=PaymentScheduleFrequency.objects.get(name__exact='Weekly'),
            weekly_dow_wed=True,
